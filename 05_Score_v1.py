...
Diane Kim
17/08/21
Version 1 - Added scores and GUI
"""
...
class Quiz:
    ...
    # Open Score GUI
    def open_score(self):
        global menu_frame, play_frame, score_frame
        menu_frame.grid_forget()
        play_frame.grid_forget()
        ...
        score_label = Label(score_frame,
                            ...
                            background="Lemon Chiffon",
                            width=20)
        score_label.grid(row=0, pady=20)

        score_text = ""
        scores.sort(reverse=True)
        for record in scores:
            score_text = score_text + f"{record[1]}\t{record[0]}/10\n"
        score_list = Label(score_frame,
                           text=score_text,
                           font=("Lucida Console", "18"),
                           bg=background)
        score_list.grid(row=1, padx=10, pady=10)
