...
Diane Kim
01/09/21
Version 3a - Add Option to quit (Option A) - Button
"""
...
class Quiz:
    ...
    def level(self):
        ...
        if level == 1:
            ...
            # Quit button
            quit_button = Button(play_frame,
                                 text="Quit",
                                 font=('lucida console', '14'),
                                 bg='gold',
                                 activebackground='light goldenrod',
                                 command=self.return_menu)
            quit_button.grid(row=8, columnspan=2, pady=5)
    ...
    # Quit function -- redirect to scoreboard
    def return_menu(self):
        global score, random_mode, mode, trial, level
        print(score)
        # Save scores
        if random_mode:
